...
static std::ostream& operator<<(std::ostream& os, const SceneType& sceneType)
{
    ...
    case SceneType::TextureMappingDebug: {
        os << "SceneType::TextureMappingDebug";
        break;
    }
    }
    return os;
...
std::optional<SceneType> deserialize(const std::string& sceneTypeStr)
{
    ...
    } else if (lowered == "texture_mapping_debug") {
        return SceneType::TextureMappingDebug;
    } else {
        return std::nullopt;
    }
}