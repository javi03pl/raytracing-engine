...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        bool bloomDebugEnabled = { false };
        int bloomIntensityValue = 10;
        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::Button("Render to file")) {
                ...
                if (result == NFD_OKAY) {
                    ...
                    setBloomDebug(bloomDebugEnabled);
                    setBloomIntensity(bloomIntensityValue);
            ...
            if (config.features.extra.enableBloomEffect) {
                ImGui::SliderInt("Bloom Intensity", &bloomIntensityValue, 0, 200);
                ImGui::Checkbox("Bloom Debug", &bloomDebugEnabled);
            }
